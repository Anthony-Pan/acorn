...
No Apple Developer account needed; codesigning is disabled for the
...
## Deploy to a real iPhone / iPad

Real-device deployment requires an Apple Developer account (free
personal accounts work for 7-day provisioning). One-time setup:

1. **Add your Apple ID to Xcode:**
   `Xcode → Settings (⌘,) → Accounts → +` → sign in
2. **Find your Team ID** (10 alphanumeric chars, e.g. `ABCDE12345`):
   In the same Accounts pane, click your Apple ID → click the team
   name in the right pane → "Team ID" is shown there. Free personal
   accounts show "(Personal Team)" — the Team ID is still 10 chars.
3. **Set the team in Xcode:** open `AcornIOS.xcodeproj`, pick the
   `AcornIOS` target → **Signing & Capabilities** tab → set **Team** to
   your account's team. Xcode auto-creates the provisioning profile.

   > ⚠️ This team selection lives in `.xcodeproj`, which is gitignored
   > and regenerated by `xcodegen` every time the project spec changes.
   > After any `xcodegen generate` run you'll need to re-select your
   > team in Xcode. If you want it to persist, you can patch
   > `App/project.yml` locally with your team ID under the `AcornIOS`
   > target's `settings.base.DEVELOPMENT_TEAM` — but don't commit that.
4. **First device build:** plug in your iPhone (cable or wireless),
   accept the "Trust this Mac" prompt on the phone, select your phone
   from Xcode's destination picker, hit ⌘R. Xcode will create the
   provisioning profile automatically (Automatic Signing).
5. **On the iPhone:** first launch will fail with "Untrusted Developer".
   Open Settings → General → VPN & Device Management → tap your
   developer profile → Trust. Then launch the app from the home
   screen.

### Free personal account caveats

- App expires after 7 days; rebuild from Xcode to refresh
- Bundle id `app.acorn.ios` may conflict if anyone else has signed
  the same id with a paid account — change `PRODUCT_BUNDLE_IDENTIFIER`
  in `App/project.yml` to something like `app.acorn.ios.dev.yourname`
- Max 3 apps signed at any one time
- No App Intents discoverability by Siri (paid account required)

### Common errors

| Symptom | Fix |
|---|---|
| `The executable is not codesigned. Signing Certificate: None` | Either (a) you haven't picked a Team in Signing & Capabilities yet (step 3), or (b) the Team is selected but no Apple Development cert exists for it on this Mac. For (b): Xcode → Settings → Accounts → click your team → **Manage Certificates...** → **+** → Apple Development. Xcode generates and installs the cert. |
| `No profiles for 'app.acorn.ios' were found` | Bundle id collision with another developer — change `PRODUCT_BUNDLE_IDENTIFIER` in `App/project.yml` to e.g. `app.acorn.ios.dev.yourname`, re-`xcodegen generate`. |
| `Untrusted Developer` on iPhone launch | Settings → General → VPN & Device Management → tap your developer profile → Trust. |
| Build succeeds but app doesn't install on device | Free account limit reached (3 apps); delete an old test build from the phone. |
| Team selection vanishes after `xcodegen generate` | Expected — the `.xcodeproj` is regenerated from spec. Either re-pick in Xcode UI, or patch `App/project.yml` `targets.AcornIOS.settings.base.DEVELOPMENT_TEAM` locally (don't commit). |
